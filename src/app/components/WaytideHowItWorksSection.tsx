...
function BriefedRiveVisual() {
  const { RiveComponent } = useRive({
    src: riveFile,
    animations: ["Timeline 1", "Timeline 2", "Timeline 3"],
    autoplay: true,
    layout: new Layout({ fit: Fit.Contain, alignment: Alignment.Bottom }),
  });
  return (
    <div className="absolute inset-0 w-full h-full px-6 pt-6">
      <RiveComponent />
    </div>
  );
}
...
const CARDS = [
  ...
  {
    bg: bg2,
    visual: <img src={overlay2} alt="" className="absolute inset-0 w-full h-full object-contain p-6" />,
  ...
  {
    bg: bg3,
    visual: <img src={overlay3} alt="" className="absolute inset-0 w-full h-full object-contain p-6" />,
